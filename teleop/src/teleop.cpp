...
private:
    ...
    double d = 0.045;       // [m] - horizontal thruster distance from centerline of CG
};
    ...
    yaw_gain(1)
{
    ...
    motorPub = nh.advertise<std_msgs::Int32[3]>("/openrov/motortarget", 1);
    lightPub = nh.advertise<std_msgs::Float32>("/openrov/light_command", 1);
    laserPub = nh.advertise<std_msgs::Int16>("/openrov/laser_toggle", 1);
    camTiltPub = nh.advertise<std_msgs::Uint16>("/openrov/camera_servo",1);
}
...
void OpenROVTeleop::joyCallback(const sensor_msgs::Joy::ConstPtr& joy)
{
    ...
    fx_d = x_gain * joy.axis[x_controllerAxis];
    fz_d = z_gain * joy.axis[z_controllerAxis];
    mz_d = yaw_gain * joy.axis[yaw_controllerAxis];

    // thruster allocation matrix
    Eigen::Matrix3d A << 1, 1, 0,
                         0, 0, 1,
                         d, -d, 0;
    ...
    Eigen::Vector3d T =  A.inverse() * F;  // A full rank <--> invertiable
